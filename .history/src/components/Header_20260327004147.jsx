...
const Header = () => {
  ...
  const { selectedBoardIndex, setData, setSelectedBoardIndex, data } =
    useContext(DataContext);

  const onEditBoard = () => setOpen(true);
  const onDeleteBoard = () => {
    if (window.confirm("Are you sure you want to delete this board?")) {
      // هنا في طريقة جديدة لإزالة عنصر من المصفوفة باستخدام toSpliced immutable method بحيث بترجع مصفوفة جديدة ولا تعدل على الاصلية
