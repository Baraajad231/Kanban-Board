...
const WorkSpace = () => {
  ...
  //   const overColumnId =
  //     over.data.current?.columnId || // بيانات الكارد
  //     (columns.find((col) => col.id === overId) ? overId : null);
  ...
  const onDragOverHandler = (event) => {
    ...
    // جلب الـ IDs
    const activeColumnId = active.data.current?.columnId;
    const overColumnId = over.data.current?.columnId || overId; // لو overId هو العمود نفسه
    ...
    setData((prev) =>
      produce(prev, (draft) => {
        ...
        const activeCol = board.columns.find((c) => c.id === activeColumnId);
        ...
        if (activeCol && overCol) {
          ...
          if (taskIndex !== -1) {
            ...
            // تحديث بيانات الكارد عشان لما يروح للعمود الجديد يعرف إنه صار فيه
