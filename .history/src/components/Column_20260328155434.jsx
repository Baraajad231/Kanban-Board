...
const Column = ({ id, title, tasks = [] }) => {
  ...
  return (
    <div className="bg-lines-light flex w-72 shrink-0 flex-col self-start rounded-lg px-2 shadow">
      ...
      <div
        ref={setNodeRef}
        className="mb-5 flex min-h-[200px] flex-col gap-5 transition-colors"
      >
        {/* {tasks.map((task) => (
          <Card
            key={task.id}
        ...
        {tasks.length > 0 ? (
          ...
          /* إذا كان فارغاً، اعرض البطاقة الشفافة */
          <div
            // هذا الـ ID يجب أن يحتوي على معرف العمود
            id={`placeholder-${id}`}
            className="border-medium-grey/20 h-20 w-full rounded-lg border-2 border-dashed opacity-0"
            // ملاحظة: الـ opacity-0 تجعلها غير مرئية لكنها موجودة في الـ DOM
          >
            {/* يمكنك وضع نص "Drop here" يظهر فقط عند السحب */}
          </div>
        )}
      </div>
      <button
