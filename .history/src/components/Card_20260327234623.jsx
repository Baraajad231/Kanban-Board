...
const Card = ({ title, columnId, taskId }) => {
  ...
  const {
    attributes, // خصائص الوصول (Accessibility)
    listeners, // مستشعرات الماوس واللمس (Events)
    setNodeRef, // المرجع للعنصر في الـ DOM
    transform, // إحداثيات الحركة (x, y)
    transition, // سرعة الحركة والأنيميشن
    isDragging, // حالة منطقية: هل البطاقة تُسحب الآن؟
  } = useSortable({ id: taskId });

  // تحويل إحداثيات المكتبة إلى ستايل CSS
  const style = {
    transform: CSS.Transform.toString(transform),
    transition,
    opacity: isDragging ? 0.3 : 1, // تلميح بصري للمكان الفارغ
  };
